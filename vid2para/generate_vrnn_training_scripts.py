...
def generate_vrnn_training_scripts():
    # 5个fold和3种噪声类型
    folds = [1, 2, 3, 4, 5]
    noise_types = ['states', 'noisy_states_5', 'noisy_states_10']
    
    print("开始生成15个VRNN训练脚本...")
    
    # 确保目录存在
    os.makedirs('vrnn_training_scripts', exist_ok=True)
    
    script_count = 0
    for fold in folds:
        for noise_type in noise_types:
            script_count += 1
            filename = f'vrnn_training_scripts/train_vrnn_fold{fold}_{noise_type}.py'
    ...
    print(f"\n总共生成了 {script_count} 个VRNN训练脚本!")
    print("\nVRNN脚本列表:")
    
    for fold in folds:
        for noise_type in noise_types:
            filename = f'train_vrnn_fold{fold}_{noise_type}.py'
            print(f"  - {filename}")
    
    print("\nVRNN新特性:")
    print("  - 基于序列的时间建模")
    print("  - 变分循环神经网络结构")
    print("  - 编码器-解码器架构适配donkey数据集")
    print("  - 物理参数推断（轴距L）")
    print("  - 状态序列监督学习")
    print("  - KL散度正则化")
    print("  - 早停和模型保存")
...
if __name__ == "__main__":
    generate_vrnn_training_scripts()